...
{
    if ($_SESSION['alert_products_not_found'])
    {
    ...
    if ($_SESSION['alert_exist_product_in_orders'])
    {
        $_SESSION['alert_exist_product_in_orders'] = false;
        echo '<div class="alert alert-danger alert-dismissible fade show text-center" role="alert">
        این محصول در یک/چند سفارش ثبت شده و قابل حذف نمی باشد.
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>';
    }
    if ($_SESSION['alert_success_delete_product'])
    {
    ...
    if ($_SESSION['alert_failure_delete_product'])
    {
        $_SESSION['alert_failure_delete_product'] = false;
        echo '<div class="alert alert-danger alert-dismissible fade show text-center" role="alert">
        عملیات حذف محصول با خطا روبرو شد.
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>';
    }
}
            ...
            <div class="col-md py-2">
                <div class="card bg-dark text-light border-light">
                    <div class="card-body text-center">
                        ...
                        <form action="products/create_product_page.php" method="post">
                            <button class="btn btn-success">افزودن محصول</button>
                        </form>
                    </div>
                </div>
            </div>

...
{
    if ($_SESSION['alert_orders_not_found'])
    {
    ...
    if ($_SESSION['alert_success_delete_order'])
    {
        $_SESSION['alert_success_delete_order'] = false;
        echo '<div class="alert alert-success alert-dismissible fade show text-center" role="alert">
        سفارش مورد نظر حذف شد.
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>'; 
    }
    if ($_SESSION['alert_failure_delete_order'])
    {
        $_SESSION['alert_failure_delete_order'] = false;
        echo '<div class="alert alert-danger alert-dismissible fade show text-center" role="alert">
        عملیات حذف سفارش مورد نظر با خطا روبرو شد.
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>'; 
    }
}
            ...
            <div class="col-md py-2">
                <div class="card bg-dark text-light border-light">
                    <div class="card-body text-center">
                        ...
                        <form action="orders/create_order_page.php" method="post">
                            <button class="btn btn-success">افزودن سفارش</button>
                        </form>
                    </div>
                </div>
            </div>
